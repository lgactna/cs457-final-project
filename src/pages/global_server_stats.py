...
def update_output(statistic: str) -> plotly.graph_objs.Figure:    
    with db_con.session_maker.begin() as session:
        result: tuple[datetime.datetime, str, float] = session.execute(
            sqlalchemy.select(models.LeagueSnapshot.ts, models.LeagueSnapshot.rank, sqlalchemy.func.avg(getattr(models.LeagueSnapshot, statistic)))
    ...
    fig = px.line(df, x='timestamp', y='average', color_discrete_map=util.RANK_TO_COLOR, color='rank')
    return fig
